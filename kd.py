...
class SrganTrainer:
    def __init__(self,
        ...
        self.teacher = generator
        self.teacher.load_model_from_file('weights/srgan/gan_generator.h5')
        self.student = generator(num_res_blocks=6)
    ...
    def train_step(self, lr, hr):
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            # Teacher model is not trained (run inference)
            sr1 = self.teacher(lr, training=False)
            # Student model is trained
            sr2 = self.generator_small(sr1, training=True)

            hr_output = self.discriminator(hr, training=True)
            sr_output = self.discriminator(sr2, training=True)

            con_loss = self._content_loss(sr1, sr2) # Get student output close to teacher output
